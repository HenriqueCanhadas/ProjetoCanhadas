...
def main():
    ...
    #tabela_merge['Data da Situação'] = pd.to_datetime(tabela_merge['Data da Situação'], format='%m/%d/%Y %H:%M').dt.strftime('%d/%m/%Y %H:%M')
    ...
    lista_pm = data_frame['Identificação da Amostra'].unique()
    ...
    for descricao_metodo in lista_descricao_metodo:
        ...
        if not tabela_merge.empty:
            ...
            colunas_desejadas = ["Identificação da Amostra", "Data da Situação", "CASNUMBER", "Análise", "Resultado", "Unidade", "Description"]
    ...
    workbook.save(r"C:\Users\henrique.canhadas\OneDrive - Servmar Ambientais\Documentos\Codigos\GitHub Tecnologia\ProjetoCanhadas\Tabelas Consulta\Banco de Dados\teste.xlsx")
